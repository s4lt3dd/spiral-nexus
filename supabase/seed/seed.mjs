...
export const OWNERS = [
  { email: "owner-a@spiralnexus.test", display_name: "Ava Owner", org_name: "Northwind Brands" },
  { email: "owner-b@spiralnexus.test", display_name: "Ben Owner", org_name: "Lumen Studio" },
];

// owner index -> trademark listings
const LISTINGS = [
  // Owner A
...
async function ensureOwner(owner) {
  ...
  const { error } = await admin
    .from("profiles")
    .update({ display_name: owner.display_name, org_name: owner.org_name })
    .eq("id", user.id);
  if (error) throw error;
  return user;
}
